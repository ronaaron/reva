...
: notnull! dup c@ 0if wordbuf ctype ."  is incomplete" cr -1 throw ;then count ;
: -cr ( a n -- )
	00; 2dup 0do
		count 
...
: desc: ( <desc>^L ) 
	...
	enquote 9996 min descbuf lplace 
	| write out the word
	langbuf @ if
...
: init
	helpdir pad place " help.db" pad +place pad count
...
: (dolibs) ( a n -- )
	...
	verbose if cr origbuf ctype cr  then
	db sql_begin
