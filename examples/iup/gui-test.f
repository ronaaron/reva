...
: quit callback  ." Ok, bye!" cr gui-close ; 

| keyboard event handler inside an editbox
| ignore the key "e"
| and replace "a" by "b"
: key-validate callback ( cb-param<handle c cstr> -- continuationcode )
  1 cb-param
...
: key-any callback ( cb-param<handle c> -- continuationcode )
  1 cb-param
  148 =if ." F6 pressed!" cr else ." other key pressed!" cr then 
  gui-default
; 
