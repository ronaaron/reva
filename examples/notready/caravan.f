...
: sort ( node n node n... -- node-max n-max...node-min n-min)
        temp @ 0do
           remains 0do                                 | pass through data
              0 pick fourth <if                        | leaving largest on stack
                 >x >x                                 | while moving
              else                                     | all lesser
...
: getmaterial ( xt-c xt-dir a1 a2 -- xt-c xt-dir a1 a2 c)
       ...
       ;then                                           | otherwise,
        fourth execute ;                               | prepare whatever's at the xt (c-wall or c-tile)
...
: build ( xt-c xt-dir a1 a2 n -- c a1 a2)
        1 do ( ...xt-dir a1 a2)
           dup w@ tile =if ( ...xt-dir a1 a2)          | check the square, is it a floor tile? if so,
              third execute                           | skip to next square,
              remains 1+
          ...
          ;then
           ...
           | .test 2 ms
           third execute ( ...xt-dir a1 a2')          | advance in specified direction
        loop  _nip ;                               | drop direction
...
: floor ( xt-tile a1 a2 -- a2 xt-tile a1)
        ...
        wide 1- 1 do ( a2 xt-f a1)                     | for the width of the building less 1,
           third i 1 ?do  E  loop ( a2 xt-f a1 a2')   | and moving one square east each iteration,
...
:: ( xt-ctile xt-cwall --)
        ...
        third obstacle? ?retry 0; ( ...a1 x'y')      | if on an obstacle, start over
