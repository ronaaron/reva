...
: start[] ( -- a)  1 rows 2 - xy>c ;                   | players' starting square
: win[] ( -- a)  1 1 xy>c ;                            | arrive here to win the game
...
: .[] ( a --)
        dup c>xy gotoxy                                | go to screen position,
...
: ramps'n'tiles ( -- a-last)
        sides
        0 1 xy>c                                       | set sides as current list
        rows 2 - 0do
           otherway swap
           repeat tiles while
           ramps
...
: slide ( c xt-dir --)
        ...
        cols 2/ rnd 1+ 0do
           dup -tileN/S? if
              drop 3drop unloop
           ;then ( a c xt a)
           third over .preview (pok) ( a c xt a)
